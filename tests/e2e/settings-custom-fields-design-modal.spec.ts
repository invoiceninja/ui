...
const INVOICE_DRAG_HELP =
  'Drag and drop fields to change their order and location';
const PRODUCT_DRAG_HELP = 'Drag and drop fields to change their order';
  ...
  await updateCompany(api, {
    ...
    settings: {
      ...
      pdf_variables: {
        ...
        [INVOICE_TARGET]: designVariables(company, INVOICE_TARGET).filter(
          (variable) => variable !== INVOICE_VARIABLE
        ),
        [PRODUCT_TARGET]: designVariables(company, PRODUCT_TARGET).filter(
          (variable) => variable !== PRODUCT_VARIABLE
        ),
      },
    },
  });
...
test('design modal keeps both the invoice and the product custom field added in a single save', async ({
  page,
  api,
}) => {
  ...
  try {
    ...
    await expect(
      dialog.getByText(INVOICE_DRAG_HELP, { exact: true })
    ).toBeVisible({ timeout: 10000 });
    await expect(
      dialog.getByText(PRODUCT_DRAG_HELP, { exact: true })
    ).toBeVisible();
    await expect(dialog.getByText(invoiceLabel, { exact: true })).toBeVisible();
    await expect(dialog.getByText(productLabel, { exact: true })).toBeVisible();
