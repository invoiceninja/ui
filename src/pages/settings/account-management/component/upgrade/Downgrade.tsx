...
export function Downgrade({ docuninja_num_users = 0 }: Props) {
  ...
  const [showChangeDocuNinjaModal, setShowChangeDocuNinjaModal] =
    useState(false);
  ...
  async function handleDocuNinjaChange(newUserCount: number) {
    ...
    try {
      await request(
        'POST',
        endpoint('/api/client/account_management/docuninja/downgrade'),
        {
    ...
    <div className="space-y-2">
      <div className="mt-4">
        ...
        <div className="flex flex-col text-center justify-between">
          <p className="text-sm">
            Need help? Please use the in app message feature to raise a support
            request.
          </p>
        </div>
      </div>
