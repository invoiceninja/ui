...
export function AndorraRegions() {
  ...
  const regions: Array<[string, TaxSetting]> = Object.entries(
    companyChanges.tax_data.regions.AD.subregions
  );
  ...
  return (
    <>
      <Element leftSide="Andorra" key="AD">
        <div className="grid grid-cols-5 gap-4">
          <div className="col-span-4">
            <SelectField
              id="tax_data.regions.AD.tax_all_subregions"
              className=""
              value={companyChanges.tax_data.regions.AD.tax_all_subregions}
              onValueChange={(value) =>
                handleChangeAndUpdateView(
                  'tax_data.regions.AD.tax_all_subregions',
                  value === 'true'
                )
              }
            >
          ...
          {!companyChanges.tax_data.regions.AD.tax_all_subregions && (
          ...
          >
            <div
              ...
              onClick={(e) => {
                ...
                divClickIntercept(
                  `tax_data.regions.AD.subregions.${value[0]}.apply_tax`
                );
              }}
            >
              <Checkbox
                id={`tax_data.regions.AD.subregions.${value[0]}`}
                value={`tax_data.regions.AD.subregions.${value[0]}.apply_tax`}
                checked={value[1].apply_tax ? true : false}
                className="flex justify-end h-6 w-6 rounded-half shadow"
                disabled={companyChanges.tax_data.regions.AD.tax_all_subregions}
