...
export function UpgradeModal({ visible, onClose, onPaymentComplete }: Props) {
  ...
  const [docuNinjaUsers, setDocuNinjaUsers] = useState(
    account?.docuninja_num_users
  );
  const [isYearly, setIsYearly] = useState(true);
  const [pricing, setPricing] = useState<PricingResponse | null>(null);
  const [currentStep, setCurrentStep] = useState<ModalStep>(
    ModalStep.PLAN_SELECTION
  );
  ...
  const getProductKeyForTier = (tierValue: number, isYearly: boolean) => {
    const tier =
      ENTERPRISE_TIERS_MAP[tierValue as keyof typeof ENTERPRISE_TIERS_MAP];
    if (!tier) return null;
  ...
  const getDocuNinjaUserOptions = () => {
    if (docuNinjaSelected) {
      ...
      const options = Array.from({ length: effectiveMaxUsers }, (_, i) => {
        const userCount = i + 1;
        return {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (
        ...
        fetchPricing(
          account?.plan as 'pro' | 'enterprise',
          account?.num_users || 1,
          docuNinjaUsers,
          isYearly
        );
        ...
        const effectiveUsers =
          selectedMainPlan === 'enterprise' ? enterpriseUsers : 1;
        fetchPricing(
          selectedMainPlan,
          effectiveUsers,
          effectiveDocuNinjaUsers,
          isYearly
        );
      }
    }, 300);
  ...
  const handlePlanToggle = (planValue: string) => {
    if (planValue === 'docuninja') {
      if (isDocuNinjaAvailable) {
        ...
        const hasExistingDocuNinjaUsers =
          (account?.docuninja_num_users || 0) > 0;
  ...
  const handleEnterpriseUserChange = (
  ...
  const fetchPricing = (
    ...
    request(
      'POST',
      endpoint('/api/client/account_management/v2/description'),
      {
        plan: plan,
        users: users,
  ...
  const handleDocuNinjaUserChange = (
    event: React.ChangeEvent<HTMLSelectElement>
  ) => {
    const newValue = parseInt(event.target.value);
    setDocuNinjaUsers(newValue);
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size={
        account?.plan == 'enterprise' || currentStep === ModalStep.PAYMENT
          ? 'regular'
          : 'large'
      }
        ...
        {currentStep === ModalStep.PLAN_SELECTION && (
          <>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div
                  className={`grid grid-cols-1 ${account?.plan == 'enterprise' ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-4`}
                >
                      ...
                      <ul className="text-sm space-y-1">
                        ...
                        {(() => {
                          ...
                          const shouldShowSelector =
                            isDocuNinjaPlan && docuNinjaSelected;
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button
                    ...
                    disabled={
                      !pricing || isLoading || pricing.pro_rata_raw <= 0
                    }
                  >
                    {t('next')}
                  </Button>
            ...
            <div className="space-y-4">
              ...
              <PaymentMethodForm
                ...
                num_users={
                  selectedMainPlan === 'enterprise'
                    ? enterpriseUsers
                    : hasExistingPlan && !selectedMainPlan
                      ? account?.num_users || 1
                      : 1
                }
                plan={
