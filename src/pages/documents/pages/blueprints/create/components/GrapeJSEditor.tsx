...
export function GrapeJSEditor({
      ...
      try {
        ...
        if (!blockManager.get('signature-placeholder')) {
          ...
          blockManager.add('signature-placeholder', {
            label:
              '<div class="gjs-block-label"><i class="fas fa-signature gjs-block__media"></i><div class="gjs-block-label">Signature</div>',
            content: {
          ...
          blockManager.add('date-placeholder', {
            label:
              '<div class="gjs-block-label"><i class="fas fa-calendar gjs-block__media"></i><div class="gjs-block-label">Date</div>',
            content: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const customStore = (editor as any).__customPagesStore as
        | {
            data: Record<string, { html: string; css: string }>;
            order: string[];
            activeId: string;
          }
        | undefined;

      const buildDoc = (html: string, css: string) =>
        `<!DOCTYPE html><html><style>${css || ''}</style></head><body>${html || ''}</body></html>`;

      const pagesPayload: Record<string, string> = {};
      if (pagesApi && pagesApi.getAll) {
        // Iterate all pages using API
        const current =
          typeof pagesApi.getActive === 'function'
            ? pagesApi.getActive()
            : (pagesApi as any).getSelected?.();
        const all = pagesApi.getAll();
        for (const pg of all) {
          // Activate each page and capture
          if (typeof (pagesApi as any).select === 'function')
            (pagesApi as any).select(pg);
          else if (typeof (pagesApi as any).setActive === 'function')
            (pagesApi as any).setActive(pg);
          const html = editor.getHtml();
          const css = editor.getCss();
        ...
        if (current) {
          if (typeof (pagesApi as any).select === 'function')
            (pagesApi as any).select(current);
          else if (typeof (pagesApi as any).setActive === 'function')
            (pagesApi as any).setActive(current);
        }
      } else if (customStore) {
        ...
        pagesPayload['page-1'] = buildDoc(
          String(editor.getHtml()),
          String(originalGetCss)
        );
      }
  ...
  return (
    <div className="h-full w-full flex flex-col">
      {/* Toolbar */}
      <div
        className="flex items-center justify-between p-3 border-b bg-white shadow-sm"
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col bg-white min-w-0">
            ...
            <div className="flex-1 relative min-w-0">
              <div
                ref={editorRef}
                className="absolute inset-0 gjs-editor w-full h-full"
              ></div>
            </div>
          </div>
        </div>
