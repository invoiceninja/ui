...
export function useInvoiceExpense() {
    ...
    const calculatedTaxRate = (expense: Expense, amount: number, default_rate: number) => {
        ...
        if(expense.calculate_tax_by_amount){
            ...
            return Math.round(((amount / (expense.amount - amount)) * 100 ) /1) /1;
    ...
    return async (expense: Expense) => {
        if (data) {
            ...
            const item: InvoiceItem = {
                ...
                tax_rate2: calculatedTaxRate(expense, expense.tax_amount2, expense.tax_rate2),
                tax_name3: expense.tax_name3,
                tax_rate3: calculatedTaxRate(expense, expense.tax_amount3, expense.tax_rate3),
            };
