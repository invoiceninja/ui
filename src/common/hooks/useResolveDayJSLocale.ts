...
export function normalizeDayJSLocale(candidate: unknown): ILocale | null {
  ...
  return isRecord(current) && typeof current.name === 'string'
    ? (current as unknown as ILocale)
    : null;
}

function loadDayJSLocale(appLanguage: string): Promise<unknown> {
...
export function useResolveDayJSLocale() {
  return async (appLanguage: string): Promise<ILocale> => {
    try {
      const locale = normalizeDayJSLocale(await loadDayJSLocale(appLanguage));

      if (locale) {
        return locale;
      }
    } catch {
      // Fall through to English if a locale chunk cannot load.
    }

    const fallback = normalizeDayJSLocale(await import('dayjs/locale/en'));

    if (!fallback) {
      throw new Error('Unable to resolve the fallback Dayjs locale');
    }
