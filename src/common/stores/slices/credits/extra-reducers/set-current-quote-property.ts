...
export const setCurrentCreditProperty = createAsyncThunk(
  'credits/setCurrentCreditProperty',
  async (payload: any, thunkApi) => {
    ...
    if (state.credits.current?.client_id) {
      client = await clientResolver.find(state.credits.current?.client_id);
    }
