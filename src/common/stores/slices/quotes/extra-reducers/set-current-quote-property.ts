...
export const setCurrentQuoteProperty = createAsyncThunk(
  'quotes/setCurrentQuoteProperty',
  async (payload: any, thunkApi) => {
    ...
    if (state.quotes.current?.client_id) {
      client = await clientResolver.find(state.quotes.current?.client_id);
    }
