...
export function TimePickerPopperSection(props: Props) {
  const { section, isSelected, handleChange } = props;

  const { timeFormatId } = useCompanyTimeFormat();

  const sectionNumbers = TimeSection.HOURS === section ? timeFormatId : 60;
  ...
  return (
    <div className="overflow-scroll scrollbar-none">
      {[...Array(sectionNumbers).keys()].map((number, index) => (
        <div
